...
resource "aws_lambda_permission" "check-number" {
  statement_id  = "InvokeCheckNumberFromApi"
...
########### POST (execute-get-voucher) ###########

resource "aws_api_gateway_resource" "execute-get-voucher" {
  rest_api_id = "${aws_api_gateway_rest_api.t-mobile-infrastructure.id}"
  parent_id   = "${aws_api_gateway_rest_api.t-mobile-infrastructure.root_resource_id}"
  path_part   = "execute-get-voucher"
}

resource "aws_api_gateway_method" "execute-get-voucher-post" {
  rest_api_id   = "${aws_api_gateway_rest_api.t-mobile-infrastructure.id}"
  resource_id   = "${aws_api_gateway_resource.execute-get-voucher.id}"
  http_method   = "POST"
  authorization = "NONE"
}

resource "aws_api_gateway_integration" "execute-get-voucher" {
  rest_api_id             = "${aws_api_gateway_rest_api.t-mobile-infrastructure.id}"
  resource_id             = "${aws_api_gateway_resource.execute-get-voucher.id}"
  http_method             = "${aws_api_gateway_method.execute-get-voucher-post.http_method}"
  integration_http_method = "POST"
  type                    = "AWS"
  uri                     = "arn:aws:apigateway:${var.region}:lambda:path/2015-03-31/functions/${aws_lambda_function.execute-get-voucher.arn}/invocations"
  credentials             = "arn:aws:iam::${var.account_id}:role/${aws_iam_role.t-mobile-role.name}"
  ...
  request_templates {
    "application/json" = <<EOF
    {
      ...
      "campaignId": $input.json('$.campaignId'),
      "stateMachineArn": "${aws_sfn_state_machine.get-voucher-state-machine.id}"
    }
    EOF
  }
...
resource "aws_api_gateway_method_response" "execute-get-voucher-post-200" {
  rest_api_id = "${aws_api_gateway_rest_api.t-mobile-infrastructure.id}"
  resource_id = "${aws_api_gateway_resource.execute-get-voucher.id}"
  http_method = "${aws_api_gateway_method.execute-get-voucher-post.http_method}"
  status_code = "200"
...
resource "aws_api_gateway_integration_response" "execute-get-voucher-post-200-response" {
  depends_on  = [ "aws_api_gateway_integration.execute-get-voucher" ]
  rest_api_id = "${aws_api_gateway_rest_api.t-mobile-infrastructure.id}"
  resource_id = "${aws_api_gateway_resource.execute-get-voucher.id}"
  http_method = "${aws_api_gateway_method.execute-get-voucher-post.http_method}"
  status_code = "${aws_api_gateway_method_response.execute-get-voucher-post-200.status_code}"
}

resource "aws_lambda_permission" "execute-get-voucher" {
  statement_id  = "AllowExecutionFromAPIGateway"
  action        = "lambda:InvokeFunction"
  function_name = "${aws_lambda_function.execute-get-voucher.arn}"
  principal     = "apigateway.amazonaws.com"
  source_arn    = "arn:aws:execute-api:${var.region}:${var.account_id}:${aws_api_gateway_rest_api.t-mobile-infrastructure.id}/*/*/*"
}

########### POST (get-voucher-sync) ###########

resource "aws_api_gateway_resource" "get-voucher-sync" {
  rest_api_id = "${aws_api_gateway_rest_api.t-mobile-infrastructure.id}"
  parent_id   = "${aws_api_gateway_rest_api.t-mobile-infrastructure.root_resource_id}"
  path_part   = "get-voucher-sync"
}

resource "aws_api_gateway_method" "get-voucher-sync-post" {
  rest_api_id   = "${aws_api_gateway_rest_api.t-mobile-infrastructure.id}"
  resource_id   = "${aws_api_gateway_resource.get-voucher-sync.id}"
  http_method   = "POST"
  authorization = "NONE"
}

resource "aws_api_gateway_integration" "get-voucher-sync" {
  rest_api_id             = "${aws_api_gateway_rest_api.t-mobile-infrastructure.id}"
  resource_id             = "${aws_api_gateway_resource.get-voucher-sync.id}"
  http_method             = "${aws_api_gateway_method.get-voucher-sync-post.http_method}"
  integration_http_method = "POST"
  type                    = "AWS"
  uri                     = "arn:aws:apigateway:${var.region}:lambda:path/2015-03-31/functions/${aws_lambda_function.get-voucher.arn}/invocations"
  credentials             = "arn:aws:iam::${var.account_id}:role/${aws_iam_role.t-mobile-role.name}"
...
resource "aws_api_gateway_method_response" "get-voucher-sync-post-200" {
  rest_api_id = "${aws_api_gateway_rest_api.t-mobile-infrastructure.id}"
  resource_id = "${aws_api_gateway_resource.get-voucher-sync.id}"
  http_method = "${aws_api_gateway_method.get-voucher-sync-post.http_method}"
  status_code = "200"
...
resource "aws_api_gateway_integration_response" "get-voucher-sync-post-200-response" {
  depends_on  = [ "aws_api_gateway_integration.get-voucher-sync" ]
  rest_api_id = "${aws_api_gateway_rest_api.t-mobile-infrastructure.id}"
  resource_id = "${aws_api_gateway_resource.get-voucher-sync.id}"
  http_method = "${aws_api_gateway_method.get-voucher-sync-post.http_method}"
  status_code = "${aws_api_gateway_method_response.get-voucher-sync-post-200.status_code}"
}

resource "aws_lambda_permission" "get-voucher-sync" {
  statement_id  = "InvokeGetVoucherSyncFromApi"
  action        = "lambda:InvokeFunction"
  function_name = "${aws_lambda_function.get-voucher.arn}"
  principal     = "apigateway.amazonaws.com"
  source_arn    = "arn:aws:execute-api:${var.region}:${var.account_id}:${aws_api_gateway_rest_api.t-mobile-infrastructure.id}/*/*/*"
}
...
resource "aws_api_gateway_deployment" "t-mobile-infrastructure" {
  depends_on = [
    ...
    "aws_api_gateway_method.execute-get-voucher-post",
    "aws_api_gateway_integration.execute-get-voucher",
    "aws_api_gateway_method.get-voucher-sync-post",
    "aws_api_gateway_integration.get-voucher-sync"
  ]
  rest_api_id = "${aws_api_gateway_rest_api.t-mobile-infrastructure.id}"
  stage_name = "dev"
}
